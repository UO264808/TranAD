...
def simple_discretize_dataset(data: torch.Tensor, n_letters: int=4):
    ...
    #    min_v = data[col_name].min()
        #print(type(data[col_name])) # Debug purposes
        # TODO: Try different number of letters
        #data[col_name] = data[col_name].apply(lambda x: str(chr(65+int((x-min_v)/(max_v-min_v)*n_letters))))
